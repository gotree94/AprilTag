...
|:----:|:----:|:----:|:----:|:----:|
| ![](tag/tag36h11_5.png) | ![](tag/tag36h11_6.png) | ![](tag/tag36h11_7.png) | ![](tag/tag36h11_8.png) | ![](tag/tag36h11_9.png) |
...
### 4.5 차량 방향 추정 및 네비게이션 — Stage 3

`cv2.minAreaRect`로 Contour의 최소 회전 사각형을 구하여 각 차량의 방향(angle)을 추정합니다.
...
**방향 각도 정규화:**
- OpenCV 기본 angle은 [-90, 0) 범위로, 실제 방향과 불일치 가능
- `width < height`일 때 90° 보정하여 long axis 기준으로 통일
- 프레임 간 각도 점프(179°→0°)를 unwrapping으로 제거하여 연속적인 각도 유지

**네비게이션 (Car 1 → Car 2):**

```
목표 벡터: dx = x2 - x1, dy = y2 - y1
Heading   = atan2(dy, dx)                         # 목표 방향 (deg)
Steer     = Heading - θ1 (normalized to ±180°)    # 필요 조향각
Distance  = sqrt(dx² + dy²)                        # 두 차량 간 거리 (mm/px)
```

### 4.6 Detector 파라미터 튜닝
...
### 5.6 문제: 차량 방향 각도 점프 (Stage 3)

`cv2.minAreaRect`로 추정한 차량 방향 각도가 특정 프레임에서 90° 또는 180° 급변하는 현상 발생.

**원인:**
- OpenCV `minAreaRect`의 angle 규칙: `width >= height`일 때 angle ∈ [-90, 0)
- Contour 형태 변화로 `width`와 `height`가 swap될 때 angle이 90° 점프
- 각도가 179° 근처에서 0°로 wrap-around (360° 주기)

**해결:**
- `width < height`일 때 angle에 90° 보정하여 long axis 기준 통일
- 프레임 간 각도 차이가 90° 초과 시 unwrapping (180° 증감)으로 연속성 유지

**결과:** Car 2 각도 179.3°→178.6°→180.0°로 부드럽게 변화 (급격한 점프 없음)
...
| `{label}_angle` | ROI별 차량 방향 각도 (minAreaRect, unwrapped) |
...
- **방향 화살표** — 자동차 박스 중심에서 long axis 방향 표시
- **각도 표시** — 자동차 박스 내 우하단에 방향 각도 (deg)
- **태그 거리** — 자동차 박스 하단에 각 태그까지의 거리(mm)
- **네비게이션 (Car 1 → Car 2)**
  - 노란색 화살표: Car 1 → Car 2 방향 벡터
  - 중간 정보창: `Target: 거리  Heading: 방향각  Steer: 필요조향각`
...
| 방향 각도 추정 | minAreaRect + unwrapping (연속 각도) |
